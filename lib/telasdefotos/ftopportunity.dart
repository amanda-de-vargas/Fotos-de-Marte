...
class FtsO extends StatefulWidget { 
  final String text;
  FtsO(this.text);
 
  @override
  State<FtsO> createState() => _FtsOState(this.text);
}

class _FtsOState extends State<FtsO> {
    int paginaAtual=1;
  List todasimagens = [];
  final String text;
  _FtsOState(this.text);

  Future<List> pageData(String sol) async {
    final response = await http.Client()
        .get(Uri.parse("http://mars-photos.herokuapp.com/api/v1/rovers/Opportunity/photos?sol="+sol));
